...
public class EventManager
{
    ...
    public void RegisterHandler<T>(string channel, EventHandler<T> handler, int priority = 0) where T : IEventArgs
    {
        ...
        if (!priorityMap.TryGetValue(priority, out var handlers)) handlers = priorityMap[priority] = [];
            ...
            if (list.Remove(handler))
            {
                if (list.Count == 0)
                {
                    ...
                    if (priorityMap.Count == 0)
                    {
                        ...
                        if (channelDict.Count == 0) _eventHandlers.Remove(typeof(T));
                    }
                }
    ...
    {
        var flag = !string.IsNullOrEmpty(channel) && _eventHandlers.TryGetValue(typeof(T), out var dictionary) &&
                   dictionary.Remove(channel) && dictionary.Count == 0;
        if (flag) _eventHandlers.Remove(typeof(T));
    }
