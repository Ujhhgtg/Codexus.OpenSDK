...
public class PacketManager
{
    public static PacketManager Instance
    {
        get
        {
            ...
            if ((packetManager = _instance) == null) packetManager = _instance = new PacketManager();
    ...
    private void RegisterDefaultPackets()
    {
        ...
        foreach (var assembly in assemblies) RegisterPacketFromAssembly(assembly);
    }
    ...
    public void RegisterPacket(RegisterPacket metadata, Type type)
    {
        ...
        for (var i = 0; i < versions.Length; i++)
        {
            ...
            if (flag5) dictionary4 = dictionary3[enumProtocolVersion] = new Dictionary<int, Type>();
            dictionary[enumProtocolVersion] = num2;
            dictionary4[num2] = type;
        }
    }

    public IPacket? BuildPacket(EnumConnectionState state, EnumPacketDirection direction, EnumProtocolVersion version,
        int packetId)
    {
        IPacket? packet;
        if (!_packets.TryGetValue(state, out var dictionary) ||
            !dictionary.TryGetValue(direction, out var dictionary2) ||
            !dictionary2.TryGetValue(version, out var dictionary3) || !dictionary3.TryGetValue(packetId, out var type))
            packet = null;
        else
            try
            {
    ...
    private readonly
        Dictionary<EnumConnectionState,
            Dictionary<EnumPacketDirection, Dictionary<EnumProtocolVersion, Dictionary<int, Type>>>> _packets = new();
