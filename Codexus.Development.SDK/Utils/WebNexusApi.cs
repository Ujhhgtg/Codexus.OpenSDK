...
public sealed class WebNexusApi : IDisposable
{
    ...
    {
        return JsonSerializer.Deserialize<BodyIn>(PostAsync("/api/PeZeroKnowledgeProof/zkp/get/check-num",
            new { dynamicPyCode, dynamicCheckSalt, gamePlayerId }).GetAwaiter().GetResult()).Body;
    }

    public string PeMcpGetStartType(string signature, string userId)
    {
        return JsonSerializer
            .Deserialize<BodyIn>(PostAsync("/api/PeZeroKnowledgeProof/zkp/get/start-type", new { signature, userId })
        ...
        Dictionary<string, string>? headers = null)
    {
        ...
        try
        {
            ...
            if (totalBytes <= 0) _progressCallback?.Invoke(id, 100);
