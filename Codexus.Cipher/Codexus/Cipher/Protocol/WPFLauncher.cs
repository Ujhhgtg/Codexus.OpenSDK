...
public class WPFLauncher : IDisposable
{
        ...
        get
        {
            ...
            if (flag)
            {
                ...
                lock (httpLock)
                {
                    ...
                    if (flag3) _http = new HttpWrapper();
                }
            }
    ...
    public WPFLauncher()
    {
        ...
        _client = new HttpWrapper("https://x19mclobt.nie.netease.com",
            delegate(HttpWrapper.HttpWrapperBuilder builder) { builder.UserAgent(userAgent); });
        _core = new HttpWrapper("https://x19obtcore.nie.netease.com:8443",
            delegate(HttpWrapper.HttpWrapperBuilder builder) { builder.UserAgent(userAgent); });
        _game = new HttpWrapper("https://x19apigatewayobt.nie.netease.com",
            delegate(HttpWrapper.HttpWrapperBuilder builder) { builder.UserAgent(userAgent); });
        _gateway = new HttpWrapper("https://x19apigatewayobt.nie.netease.com",
            delegate(HttpWrapper.HttpWrapperBuilder builder) { builder.UserAgent(userAgent); });
        _rental = new HttpWrapper("https://x19mclobt.nie.netease.com",
            delegate(HttpWrapper.HttpWrapperBuilder builder) { builder.UserAgent(userAgent); });
    }
    ...
    private async Task<ValueTuple<EntityAuthenticationOtp, string>> LoginWithCookieAsync(EntityX19CookieRequest cookie)
    {
        ...
        if (flag) await _sdk.AuthSession(cookie.Json);
    ...
    private async Task<EntityLoginOtp> LoginOtpAsync(EntityX19CookieRequest cookieRequest)
    {
        var httpResponseMessage =
            await _core.PostAsync("/login-otp", JsonSerializer.Serialize(cookieRequest, DefaultOptions));
        var text2 = await httpResponseMessage.Content.ReadAsStringAsync();
        var entity = JsonSerializer.Deserialize<Entity<JsonElement?>>(text2);
        if (entity == null) throw new Exception("Failed to deserialize: " + text2);
    ...
    private async Task<EntityAuthenticationOtp> AuthenticationOtpAsync(EntityX19CookieRequest cookieRequest,
        EntityLoginOtp otp)
    {
        ...
        if (array2 == null) throw new Exception("Cannot decrypt data");
    ...
    private async Task<Entities<EntityNetGameItem>> GetAvailableNetGamesAsync(string userId, string userToken,
        int offset, int length)
    {
        ...
        var httpResponseMessage = await _game.PostAsync("/item/query/available", body,
    ...
    private async Task<Entities<EntityQueryNetGameItem>> QueryNetGameItemByIdsAsync(string userId, string userToken,
        string[] ids)
    {
        ...
        var httpResponseMessage = await _game.PostAsync("/item/query/search-by-ids", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entity<EntityQueryNetGameDetailItem>> QueryNetGameDetailByIdAsync(string userId,
        string userToken, string gameId)
    {
        var body = JsonSerializer.Serialize(new EntityQueryNetGameDetailRequest
        {
            ItemId = gameId
        }, DefaultOptions);
        var httpResponseMessage = await _game.PostAsync("/item-details/get_v2", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entities<EntityGameCharacter>> QueryNetGameCharactersAsync(string userId, string userToken,
        string gameId)
    {
        ...
        var httpResponseMessage = await _game.PostAsync("/game-character/query/user-game-characters", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
        ...
        string gameId)
    {
        ...
        var httpResponseMessage = await _game.PostAsync("/item-address/get", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    public async Task<Entity<EntityQuerySearchByGameResponse>> GetGameCoreModListAsync(string userId, string userToken,
        EnumGameVersion gameVersion, bool isRental)
    {
        ...
        var httpResponseMessage = await _game.PostAsync("/game-auth-item-list/query/search-by-game", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    public async Task<Entities<EntityComponentDownloadInfoResponse>> GetGameCoreModDetailsListAsync(string userId,
        string userToken, List<ulong> gameModList)
    {
        var body = JsonSerializer.Serialize(new EntitySearchByIdsQuery
        {
            ItemIdList = gameModList
        }, DefaultOptions);
        var httpResponseMessage = await _game.PostAsync("/user-item-download-v2/get-list", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entities<EntitySkin>> GetFreeSkinListAsync(string userId, string userToken, int offset,
        int length = 20)
    {
        ...
        var httpResponseMessage = await _gateway.PostAsync("/item/query/available", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entities<EntitySkin>> QueryFreeSkinByNameAsync(string userId, string userToken, string name)
    {
        ...
        var httpResponseMessage = await _gateway.PostAsync("/item/query/search-by-keyword", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entities<EntitySkin>> GetSkinDetailsAsync(string userId, string userToken,
        Entities<EntitySkin> skinList)
    {
        ...
        var httpResponseMessage = await _gateway.PostAsync("/item/query/search-by-ids", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<EntityResponse> PurchaseSkinAsync(string userId, string userToken, string entityId)
    {
        ...
        var httpResponseMessage = await _gateway.PostAsync("/user-item-purchase", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<EntityResponse> SetSkinAsync(string userId, string userToken, string entityId)
    {
        ...
        var httpResponseMessage = await _gateway.PostAsync("/user-game-skin-multi", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    public List<EntityUserGameTexture> GetSkinListInGame(string userId, string userToken,
        EntityUserGameTextureRequest entity)
    {
        return GetSkinListInGameAsync(userId, userToken, entity).GetAwaiter().GetResult();
    }

    private async Task<List<EntityUserGameTexture>> GetSkinListInGameAsync(string userId, string userToken,
        EntityUserGameTextureRequest entity)
    {
        var body = JsonSerializer.Serialize(entity, DefaultOptions);
        var httpResponseMessage = await _game.PostAsync("/user-game-skin/query/search-by-type", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    {
        var httpResponseMessage = await _game.PostAsync("/game-character", JsonSerializer.Serialize(
        ...
        if (flag) throw new Exception("Failed to create character");
    }
    ...
    public async Task<EntityAuthenticationUpdate> AuthenticationUpdateAsync(string userId, string userToken)
    {
        ...
        var httpResponseMessage = await _core.PostAsync("/authentication/update", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
    ...
    private async Task<Entities<EntityNetGameItem>> QueryNetGameWithKeywordAsync(string userId, string userToken,
        string keyword)
    {
        var httpResponseMessage = await _game.PostAsync("/item/query/search-by-keyword", JsonSerializer.Serialize(
                new EntityNetGameKeyword
                {
                    Keyword = keyword
                }, DefaultOptions),
            delegate(HttpWrapper.HttpWrapperBuilder builder)
    ...
    public Entity<EntityCoreLibResponse> GetMinecraftClientLibs(string userId, string userToken,
        ...
        EnumGameVersion? gameVersion = null)
    {
        ...
        var httpResponseMessage = await _client.PostAsync("/game-patch-info", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    public Entity<EntityComponentDownloadInfoResponse> GetNetGameComponentDownloadList(string userId, string userToken,
        string gameId)
    {
        return GetNetGameComponentDownloadListAsync(userId, userToken, gameId).GetAwaiter().GetResult();
    }

    private async Task<Entity<EntityComponentDownloadInfoResponse>> GetNetGameComponentDownloadListAsync(string userId,
        string userToken, string gameId)
    {
        ...
        var httpResponseMessage = await _client.PostAsync("/user-item-download-v2", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entities<EntityRentalGame>> GetRentalGameListAsync(string userId, string userToken, int offset)
    {
        ...
        var httpResponseMessage = await _rental.PostAsync("/rental-server/query/available-public-server", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entities<EntityRentalGamePlayerList>> GetRentalGameRolesListAsync(string userId,
        string userToken, string entityId)
    {
        ...
        var httpResponseMessage = await _rental.PostAsync("/rental-server-player/query/search-by-user-server", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    public Entity<EntityRentalGamePlayerList> AddRentalGameRole(string userId, string userToken, string serverId,
        string roleName)
    {
        return AddRentalGameRoleAsync(userId, userToken, serverId, roleName).GetAwaiter().GetResult();
    }

    private async Task<Entity<EntityRentalGamePlayerList>> AddRentalGameRoleAsync(string userId, string userToken,
        string serverId, string roleName)
    {
        ...
        var httpResponseMessage = await _rental.PostAsync("/rental-server-player", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entity<EntityRentalGamePlayerList>> DeleteRentalGameRoleAsync(string userId, string userToken,
        string entityId)
    {
        ...
        var httpResponseMessage = await _rental.PostAsync("/rental-server-player/delete", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    public Entity<EntityRentalGameServerAddress> GetRentalGameServerAddress(string userId, string userToken,
        string entityId, string pwd = null)
    {
    ...
    private async Task<Entity<EntityRentalGameServerAddress>> GetRentalGameServerAddressAsync(string userId,
        string userToken, string entityId, string pwd = null)
    {
        ...
        var httpResponseMessage = await _rental.PostAsync("/rental-server-world-enter/get", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entity<EntityRentalGameDetails>> GetRentalGameDetailsAsync(string userId, string userToken,
        string entityId)
    {
        ...
        var httpResponseMessage = await _rental.PostAsync("/rental-server-details/get", body,
            delegate(HttpWrapper.HttpWrapperBuilder builder)
            {
                builder.AddHeader(TokenUtil.ComputeHttpRequestToken(builder.Url, builder.Body, userId, userToken));
            });
    ...
    private async Task<Entities<EntityRentalGame>> SearchRentalGameByNameAsync(string userId, string userToken,
        string worldId)
    {
