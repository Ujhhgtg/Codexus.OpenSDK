...
public class Com4399
{
	...
	private async Task<string> OAuthDevice()
	{
		var body = new ParameterBuilder().Append("usernames", "").Append("top_bar", "1").Append("state", "")
			...
			.FormUrlEncode();
		var httpResponseMessage = await _4399Api.PostAsync("/openapiv2/oauth.html", body, "application/x-www-form-urlencoded");
	...
	public async Task<string> LoginAndAuthorize(string username, string password,  string captcha = null,  string captchaId = null, int retry = 0)
	{
		...
		var parameterBuilder = new ParameterBuilder().Append("_d", _deviceIdentifier).Append("access_token", "").Append("aid", "")
		...
		else
		{
			...
			{
				parameterBuilder2.Append("captcha", captcha);
			}
			...
			else
			{
				...
				text6 = _mgbSdk.GenerateSAuth(Guid.NewGuid().ToString("N").ToUpper(), "", result.Uid.ToString(), result.State, "", "4399com", "ad");
			}
		}
		return text6;
	}
