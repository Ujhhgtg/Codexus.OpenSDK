...
public static class StreamExtensions
{
    public static async Task<MemoryStream> ReadSteamWithInt16Async(this NetworkStream stream)
    {
        ...
        var flag = num3 != 2;
        if (flag) throw new EndOfStreamException("Could not read the length prefix.");
        var num = BitConverter.ToInt16(lengthBytes, 0);
        if (num < 0) throw new InvalidDataException("Length cannot be negative.");
        ...
        for (var remainingBytes = (int)num; remainingBytes > 0; remainingBytes -= num2)
        {
            ...
            num2 = num4;
            if (num2 == 0) throw new EndOfStreamException("End of stream reached before reading complete data.");
