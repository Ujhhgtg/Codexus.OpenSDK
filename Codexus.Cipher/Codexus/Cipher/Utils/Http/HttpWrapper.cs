    ...
    : IDisposable
{
    ...
    public async Task<HttpResponseMessage> PostAsync(string url, string body, string contentType = "application/json",
        Action<HttpWrapperBuilder> block = null)
    {
        ...
        if (flag) request.Version = version;
        ...
        else
        {
            ...
            if (extension1 != null) extension1(builder);
            block(builder);
    ...
    public class HttpWrapperBuilder(string domain, string url, string body)
    {
        ...
        {
            foreach (var keyValuePair in headers) _headers.Add(keyValuePair.Key, keyValuePair.Value);
            return this;
        }
