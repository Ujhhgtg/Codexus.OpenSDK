...
public static class XxTeaUtil
{
    ...
    private static byte[] ToByteArray(long[] data)
    {
        ...
        foreach (var num in data) list.AddRange(BitConverter.GetBytes(num));
        var num2 = list.Count - 1;
        while (num2 >= 0 && list[num2] == 0) num2--;
    ...
    private static string ToHexString(long[] data)
    {
        ...
        foreach (var num in data) stringBuilder.Append(num.ToString("x16"));
    ...
    private static long[] FromHexString(string hex)
    {
        ...
        var array = new long[num];
        for (var i = 0; i < num; i++) array[i] = Convert.ToInt64(hex.Substring(i * 16, 16), 16);
        return array;
    }
