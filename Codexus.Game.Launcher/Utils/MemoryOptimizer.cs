...
public partial class MemoryOptimizer : IDisposable
{
    ...
    private static partial IntPtr OpenProcess(uint dwDesiredAccess, [MarshalAs(UnmanagedType.Bool)] bool bInheritHandle,
        uint dwProcessId);
    ...
    public static MemoryOptimizer GetInstance()
    {
        ...
        using (Lock.EnterScope())
        {
            ...
            if ((memoryOptimizer = _instance) == null) memoryOptimizer = _instance = new MemoryOptimizer();
    ...
    private void OptimizeCallback(object? state)
    {
        if (_disposed || !RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            ...
            try
            {
                ...
                {
                    foreach (var process in minecraftProcesses) OptimizeProcess(process);
    ...
    private static bool IsMinecraftProcess(Process process)
    {
        ...
        try
        {
            ...
            flag = !string.IsNullOrEmpty(commandLine) && SourceArray.Any(keyword =>
                commandLine.Contains(keyword, StringComparison.OrdinalIgnoreCase));
        }
        catch (Exception ex)
        {
    ...
    private static string? GetCommandLine(Process process)
    {
        try
        {
            ...
            var managementObjectSearcher =
                new ManagementObjectSearcher(defaultInterpolatedStringHandler.ToStringAndClear());
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        while (enumerator.MoveNext())
                        {
                            ...
                            var managementObject = (ManagementObject)(managementBaseObject is ManagementObject
                        ...
                        if (!flag)
                            try
                            {
                                ...
                                if (flag2)
                                {
                                    ...
                                    Log.Information(
                                        "[Memory Optimizer] Process ID: {ProcessId} - Memory Before: {BeforeMemory} MB, After: {AfterMemory} MB",
                                        process.Id, num, num4);
    ...
    private void CleanupExitedProcesses()
    {
        ...
        if (!disposed)
            using (_lockObject.EnterScope())
            {
                ...
                foreach (var num2 in list) _processedIds.Remove(num2);
                var flag = list.Count > 0;
                if (flag) Log.Information("[Memory Optimizer] Cleaned up {Count} exited processes", list.Count);
            }
    }
        ...
        if (!disposed)
            using (Lock.EnterScope())
            {
                ...
                if (flag)
                {
                    ...
                    if (optimizationTimer != null) optimizationTimer.Dispose();
    ...
    [
        "minecraft", "net.minecraft", "launchwrapper", "forge", "fabric", "quilt", "optifine", ".minecraft", "versions",
        "libraries"
    ];
}