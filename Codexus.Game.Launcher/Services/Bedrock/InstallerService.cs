...
public static class InstallerService
{
    ...
    private static (string BasePath, string ArchivePath, string HashPath, string ExecutablePath) GetInstallationPaths()
    {
        ...
        return (cppGamePath, Path.Combine(cppGamePath, "mc_base.7z"), Path.Combine(cppGamePath, "minecraft_pe.md5"),
            Path.Combine(cppGamePath, "windowsmc/Minecraft.Windows.exe"));
    }
        ...
        (string BasePath, string ArchivePath, string HashPath, string ExecutablePath) paths)
    {
        ...
        if (flag)
            ...
            try
            {
                ...
                flag2 = string.Equals(Convert.ToHexStringLower(array), "50ac5016023c295222b979565b9c707b",
    ...
    private static IProgress<SyncProgressBarUtil.ProgressReport> CreateProgressReporter(
        ...
        IProgress<SyncProgressBarUtil.ProgressReport> progress)
    {
        return await DownloadUtil.DownloadAsync(
            ...
            archivePath, delegate(uint percentage)
            {
    ...
    private static async Task<bool> ValidatePackageAsync(string filePath, string expectedMd5)
    {
        ...
        if (flag) throw new ArgumentException("Expected MD5 hash cannot be null or empty", "expectedMd5");
        var flag2 = !File.Exists(filePath);
        if (flag2) throw new FileNotFoundException("File not found: " + filePath, filePath);
        var array = await File.ReadAllBytesAsync(filePath);
        return string.Equals(Convert.ToHexStringLower(MD5.HashData(array)), expectedMd5,
            StringComparison.OrdinalIgnoreCase);
    }
