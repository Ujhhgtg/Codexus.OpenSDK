...
public sealed class LauncherService : IDisposable
{
		...
		try
		{
			...
			if (!disposed)
			{
				...
				if (!_disposed)
				{
					...
					if (!_disposed)
					{
						await StartGameProcessAsync(num).ConfigureAwait(false);
					}
				}
			}
		}
	...
	private Task<int> LaunchProxyAsync()
	{
		...
		try
		{
			_rakNet = RakNetLoader.ConstructLoader().Create(text, Entity.AccessToken, Entity.GameId, Convert.ToUInt32(Entity.UserId), _userToken, Entity.GameName, Entity.RoleName, availablePort, availablePort2, flag);
		}
		catch (Exception ex)
		{
