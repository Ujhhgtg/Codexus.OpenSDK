    ...
    EnumGameVersion gameVersion)
{
    ...
    private void HandleAuthenticationNewVersion(byte[] data)
    {
        ...
        if (flag)
        {
            ...
            if (flag2) SendControlData(array);
            Log.Information(
    ...
    private async Task ProcessPlayerSkin(EntityOtherEnterWorldMsg msg)
    {
        ...
        {
            if (skin?.SkinId is not { Length: > 5 }) continue;
            ...
            try
            {
                ...
                if (availableUser == null) continue;

                var getComponents = _getComponents;
                if (getComponents == null) continue;
                ...
                if (response is not { Code: 0 }) continue;
        ...
        EnumSkinMode skinMode)
    {
        ...
        if (payload != null) SendControlData(payload);
    }
    ...
    private void OnCheckPlayerMsg(byte[] data)
    {
        ...
        Log.Information<string, string>("[RPC] Event received -> {event} with data {Message}", "Player Message Check",
            entityCheckPlayerMessage != null ? entityCheckPlayerMessage.Message : null);
    }
    ...
    {
        foreach (var array in _sendCache) SendControlData(array);
        _sendCache.Clear();
    }
    ...
    public delegate Entity<EntityComponentDownloadInfoResponse> GetComponents(string userId, string userToken,
        string str);

    public delegate List<EntityUserGameTexture> GetSkinList(string userId, string userToken,
