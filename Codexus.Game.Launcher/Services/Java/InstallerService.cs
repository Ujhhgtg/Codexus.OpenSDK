...
{
    public static async Task<bool> PrepareMinecraftClient(string userId, string userToken, WPFLauncher wpfLauncher,
        EnumGameVersion gameVersion)
    {
        ...
        if (flag) throw new Exception("Failed to fetch base package: " + minecraftClientLibs.Message);
        await ProcessPackage(minecraftClientLibs.Data.Url, zipPath, PathUtil.GameBasePath, md5Path,
            minecraftClientLibs.Data.Md5, "base package");
        ...
        await ProcessPackage(versionResult.Data.Url, versionZip, PathUtil.GameBasePath, versionMd5File,
            versionResult.Data.Md5, versionName + " package");
        await ProcessPackage(versionResult.Data.CoreLibUrl, libZip, PathUtil.CachePath, libMd5File,
            versionResult.Data.CoreLibMd5, versionName + " libraries");
        ...
        string label)
    {
        ...
        if (File.Exists(md5Path))
        {
            ...
            if (currentMd5 == md5) return; // Already installed and matches MD5
        }
        ...
        IProgress<SyncProgressBarUtil.ProgressReport> uiProgress =
            new Progress<SyncProgressBarUtil.ProgressReport>(update =>
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (flag8)
                        {
                            ...
                            var text14 = Path.Combine(PathUtil.GameBasePath, ".minecraft",
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            var flag15 = fileName.StartsWith("modlauncher-") &&
                                                         fileName.Contains("10.2.1");
                                            if (flag15)
                                            {
        ...
        bool isRental)
    {
        ...
        var progressBar = new SyncProgressBarUtil.ProgressBar(100);
        IProgress<SyncProgressBarUtil.ProgressReport> progressReporter =
            new Progress<SyncProgressBarUtil.ProgressReport>(update =>
                progressBar.Update(update.Percent, update.Message));
    ...
    public static string PrepareGameRuntime(string userId, string gameId, string roleName, EnumGType gameType)
    {
        ...
        if (flag) Directory.CreateDirectory(text2);
        ...
        if (flag3) FileUtil.DeleteDirectorySafe(text5);
    ...
    public static void InstallNativeDll(EnumGameVersion gameVersion)
    {
        try
        {
            ...
            var flag = !Directory.Exists(text2);
            if (flag) FileUtil.CreateDirectorySafe(text2);
            var flag2 = !File.Exists(text);
            if (flag2) throw new Exception("Native dll not found: " + text);
