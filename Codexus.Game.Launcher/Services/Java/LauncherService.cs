...
public sealed class LauncherService : IDisposable
{
	...
	private IProgress<EntityProgressUpdate> CreateProgressHandler()
	{
		...
		var uiProgress = new SyncCallback<SyncProgressBarUtil.ProgressReport>(delegate(SyncProgressBarUtil.ProgressReport update)
		{
	...
	{
		_authLibProtocol = new AuthLibProtocol(IPAddress.Parse("127.0.0.1"), _socketPort, JsonSerializer.Serialize<EntityModsList>(_modList), Entity.GameVersion, Entity.AccessToken);
