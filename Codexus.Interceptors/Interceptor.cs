    ...
    string nickName)
{
    ...
    public static Interceptor CreateInterceptor(EntitySocks5 socks5, string modInfo, string gameId, string serverName,
        string serverVersion, string forwardAddress, int forwardPort, string nickName, string userId, string userToken,
        Action<string>? onJoinServer = null, string localAddress = "127.0.0.1", int localPort = 6445)
    {
        ...
        var interceptor = new Interceptor(multithreadEventLoopGroup, multithreadEventLoopGroup2, serverName,
            serverVersion, forwardAddress, forwardPort, localAddress, availablePort, nickName);
        var serverBootstrap = new ServerBootstrap().Group(multithreadEventLoopGroup, multithreadEventLoopGroup2)
            .Channel<TcpServerSocketChannel>().Option(ChannelOption.SoReuseaddr, true)
            ...
            .ChildHandler(new ActionChannelInitializer<IChannel>(channel =>
            {
                channel.Pipeline.AddLast("splitter", new MessageDeserializer21Bit()).AddLast("handler",
                        new ServerHandler(interceptor, socks5, modInfo, gameId, forwardAddress, forwardPort, nickName,
                            userId, userToken, onJoinServer)).AddLast("pre-encoder", new MessageSerializer21Bit())
        ...
        serverBootstrap.BindAsync().ContinueWith(delegate(Task<IChannel> task)
        {
            ...
            if (isCompletedSuccessfully) interceptor.Channel = task.Result;
    ...
    public async Task ChangeForwardAddressAsync(string newAddress, int newPort)
    {
        ...
        ForwardPort = newPort;
        UdpBroadcaster?.Stop();
        UdpBroadcaster = new UdpBroadcaster("224.0.2.60", 4445, LocalPort, ForwardAddress, NickName,
            ServerVersion.Contains("1.8.") || ServerVersion.Contains("1.7."));
    ...
    public static void EnsureLoaded()
    {
        ...
        if (flag) throw new InvalidOperationException("Should never call CheckIsLoaded()");
    }
