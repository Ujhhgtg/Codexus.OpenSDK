...
public class CHandshake : IPacket
{
    ...
    public bool HandlePacket(GameConnection connection)
    {
        ...
        else
        {
            ...
            Log.Debug("Original address: {ServerAddress}", [
                Convert.ToBase64String(Encoding.UTF8.GetBytes(ServerAddress))
            ]);
            ServerPort = (ushort)connection.ForwardPort;
            var protocolVersion = connection.ProtocolVersion;
            var text = protocolVersion > EnumProtocolVersion.V1180
                ?
                protocolVersion <= EnumProtocolVersion.V1206
                    ? connection.ForwardAddress + "\0FML3\0"
                    : connection.ForwardAddress + "\0FORGE"
                : protocolVersion <= EnumProtocolVersion.V1122
                    ? connection.ForwardAddress + "\0FML\0"
                    : connection.ForwardAddress + "\0FML2\0";
            ServerAddress = text;
